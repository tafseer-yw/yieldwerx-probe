...
const GIT_DENY = [
  { id: "force-push", label: "force push - overwrites history other clones already have",
    re: rx(String.raw`push\b(?=[^|;&]*\s(?:--force\b(?!-with-lease)|-f\b))`) },
  { id: "plus-refspec-push", label: "force push via a +refspec - the same overwrite, spelled differently",
    re: rx(String.raw`push\b[^|;&]*\s\+[\w./-]+:`) },
  { id: "push-mirror", label: "mirror push - replaces every ref on the remote",
    re: rx(String.raw`push\b[^|;&]*\s--mirror\b`) },
  { id: "filter-branch", label: "history rewrite across the whole repository",
    re: rx(String.raw`(?:filter-branch|filter-repo)\b`) },
  { id: "reflog-expire", label: "reflog expiry - removes the safety net that makes commits recoverable",
    re: rx(String.raw`reflog\s+expire\b`) },
];

/** Destroys work that exists nowhere else. */
const GIT_ASK = [
  { id: "reset-hard", label: "hard reset - discards every uncommitted change", re: rx(String.raw`reset\b[^|;&]*\s--hard\b`) },
  { id: "clean-force", label: "forced clean - deletes untracked files, including ones never committed",
    re: rx(String.raw`clean\b[^|;&]*\s(?:-[a-zA-Z]*f[a-zA-Z]*\b|--force\b)`) },
  { id: "checkout-discard", label: "discards working-tree changes",
    re: rx(String.raw`(?:checkout|restore)\b[^|;&]*\s(?:--\s+)?\.(?:\s|$)`) },
  { id: "branch-force-delete", label: "force branch deletion - drops unmerged commits", re: rx(String.raw`branch\b[^|;&]*\s-D\b`) },
  { id: "stash-drop", label: "stash removal - stashed work has no other copy", re: rx(String.raw`stash\s+(?:drop|clear)\b`) },
  { id: "force-with-lease", label: "force push with lease - safer, still a history rewrite",
...
export function stripProsePayloads(command) {
  return String(command ?? "")
    ...
    .replace(/((?:^|\s)-[a-zA-Z]*m|(?:^|\s)--message)(\s*=?\s*)(["'])(?:\\.|(?!\3)[\s\S])*?\3/g, "$1 MSG");
}
...
export function targetBranches(clause) {
  const out = [];
  for (const m of clause.matchAll(/(?:^|\s)(?:\+)?(?:refs\/heads\/)?([A-Za-z0-9_][\w.-]*(?:\/[\w.-]+)*)(?=\s|:|$)/g)) out.push(m[1]);
  for (const m of clause.matchAll(/(?:^|\s)\+?:(?:refs\/heads\/)?([\w.-]+(?:\/[\w.-]+)*)/g)) out.push(m[1]);
  return out;
}
...
export function blastRadiusVerdict({ command }) {
  ...
  for (const clause of clausesOf(command)) {
    for (const rule of GIT_DENY) if (rule.re.test(clause)) hits.push({ decision: "deny", ...rule, clause });
    if (DELETE_REMOTE.test(clause)) {
      const branches = targetBranches(clause).filter((b) => PROTECTED_BRANCH.test(b));
      if (branches.length) {
        hits.push({ decision: "deny", id: "delete-remote-branch", label: `remote branch deletion (${branches.join(", ")})`, clause });
      }
    }
    for (const rule of GIT_ASK) if (rule.re.test(clause)) hits.push({ decision: "ask", ...rule, clause });
